...
TEST(SquareGridTest, ConstructorAndBasics) {
	...
	math::function::SquareGrid<float, float> grid(2, 2, 1.0);
	...
	math::function::SquareGrid<float, float> larger_grid(size, size, spacing, zero);
...
void display_grid(const math::function::SquareGrid<T,E>& grid) {
...
TEST(SquareGridTest, GradientTests) {
	...
	// Quadratic function.
	math::function::SquareGrid<float, float> small(size, size, step);
	for (unsigned i = 0; i < size; ++i) {
		for (unsigned j = 0; j < size; ++j) {
